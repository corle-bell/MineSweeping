    ...
    public partial class Prefs
    {
        ...
        public class QuickAccessBarManager : StandalonePrefManager<QuickAccessBarManager>
        {
            ...
            private void DrawTypeFields(QuickAccessItem item, ref Rect lineRect)
            {
                ...
                if (!item.isBasicType)
                {
                    EditorGUI.HelpBox(new Rect(lineRect){height = HELP_HEIGHT}, "Available in uContext Pro.", MessageType.Warning);
                    lineRect.y += HELP_HEIGHT + 2;
                }
#endif
...
#if !UCONTEXT_PRO
                if (item.expanded && !item.isBasicType) height += HELP_HEIGHT + 2;
#endif
