...
namespace InfinityCode.uContext.Windows
{
    public class EditorIconsBrowser : EditorWindow
    {
        ...
        {
            if (OnExport != null) OnExport(data as string);
            else if (Prefs.proHighlight)
            {
                if (EditorUtility.DisplayDialog("Warning", "Export built-in icons available in uContext Pro.", "Open uContext Pro", "Cancel"))
                {
                    Links.OpenPro();
                }
            }
