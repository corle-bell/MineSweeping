...
public static class FastPoissonTools
{
    ...
    public static List<Vector2> Sample2D(int seed, float width, float height, float r, int k = 30)
    {
        ...
        // 随机选一个起始点
        var x0 = new Vector2(UnityEngine.Random.Range(0, width), UnityEngine.Random.Range(0, height));
        ...
        {
            // 随机选一个待处理的点xi
            var xi_idx = active_list[UnityEngine.Random.Range(0, active_list.Count)]; // 区间是[0,1)，不用担心溢出。
            var xi = cells[xi_idx];
            var found = false;
