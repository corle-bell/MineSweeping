...
public class DrawPhycisLine2D : MonoBehaviour
{
    [EnumName("线条绘制")]
    ...
    private List<CircleCollider2D> colliderCache = new List<CircleCollider2D>();
    ...
    {
        CircleCollider2D last = null;
        CircleCollider2D add = null;
        Vector3 lastPos = Vector3.zero;
        if (colliders.Count > 0)
        {
            last = colliders[colliders.Count - 1] as CircleCollider2D;
    ...
    //添加碰撞体
    CircleCollider2D AddColliderObj()
    {
        if(colliderCache.Count>1)
        {
            var ret = colliderCache[0] as CircleCollider2D;
        ...
        return tmp.AddComponent<CircleCollider2D>();
    }
